...
class CameraTwoFragment : BaseFragment() {
    private val oritentionArray = SparseIntArray()



    init {
        oritentionArray.append(Surface.ROTATION_0, 90)
        oritentionArray.append(Surface.ROTATION_90, 0)
        oritentionArray.append(Surface.ROTATION_180, 270)
        oritentionArray.append(Surface.ROTATION_270, 180)
    }
    ...
    override fun getLayoutId(): Int {
        return  R.layout.fragment_camera_two
    }

    override fun initWidget() {
        super.initWidget()
        mTextureView = view?.findViewById(R.id.texture)
        mTakePhoto = view?.findViewById(R.id.takePhoto)
    }
    ...
    private fun captureStillPicture() {
        try {
            ...
            captureRequestBuild.set(CaptureRequest.JPEG_ORIENTATION, rotation?.let { oritentionArray.get(it) })
            //停止连续取景？
    ...
    private fun createCameraPreviewSession() {
        try {
            ...
            cameraDevice.createCaptureSession(listOf(texture, imageReader.surface) as MutableList<Surface>, object : CameraCaptureSession.StateCallback() {
